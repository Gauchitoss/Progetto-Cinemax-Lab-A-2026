...
// ======================================================
        ...
        public static void salvaSuFile(){
            try(PrintWriter pw = new PrintWriter(new FileWriter(FILE_PATH))){ // creazione PrintWriter e FileWriter
                pw.println("data_proiezione|ora_proiezione|titolo_film|genere|regista|anno|durata_minuti|eta_minima|prezzo_biglietto");
                for(Proiezione p: listaProiezioni){
                    pw.printf(java.util.Locale.US, "%s|%s|%s|%s|%s|%d|%d|%d|%.2f%n", p.getData(),p.getOra(), p.getTitolo(), p.getGenere(), p.getRegista(), p.getAnno(), p.getDurata(), p.getEtaMin(), p.getPrezzo()); /* uso printf al posto di println per poter sfruttare i sgenaposto
            ...
            try(BufferedReader br = new BufferedReader(new FileReader(file))){ //Creazione BufferdReader e FileReader
                ...
                while((riga = br.readLine()) != null){ //lettura del file riga per riga
                    try{
                        ...
                        if (colonna.length < 9) { //controllo di sicurezza
                        ...
                        LocalDate data = LocalDate.parse(colonna[0], formattaData);
                        Proiezione proiezione = new Proiezione(data, colonna[1], colonna[2], colonna[3], colonna[4], anno, durata, etaMin, prezzo); //creazione oggetto proiezione
